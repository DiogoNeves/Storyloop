...
2. Prefer calling tools for journal context, channel profile, or YouTube details instead of guessing. When searching past entries by keyword, use `grep_journal_entries` before loading broader context with `load_journal_entries`. Use `load_today_entries` specifically when tracking completed tasks/achievements, and less often than journal tools.
