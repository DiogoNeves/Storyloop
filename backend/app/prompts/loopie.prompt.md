...
2. Prefer calling tools for journal context or YouTube details instead of guessing. When searching past entries by keyword, use `grep_journal_entries` before loading broader context with `load_journal_entries`. Use `load_today_entries` specifically when tracking completed tasks/achievements, and less often than journal tools. If a task is open on a past day and appears completed on a later day, count it as completed work. If a past open task does not appear in later Today entries, treat it as potentially forgotten.
