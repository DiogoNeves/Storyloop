...
- This repository is a FastAPI + React project (not a monorepo) with top-level `backend/` and `frontend/` applications.
- Core backend services currently live under `backend/app/services/` and database helpers in `backend/app/db_helpers/`; these map to the rubric’s side-effect boundary responsibilities.
- Domain-style organization is emerging (`backend/services`, `backend/db_helpers`, frontend API/hooks), so strict `apps/domains/shared` naming is treated as a directional preference, not a hard requirement.
- Frontend code should still separate pure UI rendering and transformation logic from network effects (`api/` and external services).
- Explicit exceptions:
  - Existing architecture/docs (`backend/app/`, `frontend/src/`) should remain stable unless a change requires larger refactors.
  - `PydanticAI` and streaming agent code may keep orchestration-local patterns where readability or framework ergonomics outweigh strict boundary granularity.
