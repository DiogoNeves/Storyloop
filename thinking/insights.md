...
Each component is normalized (z-score) against the creator’s rolling baseline (e.g., the last _N_ videos) and scaled to a 0–100 range for presentation.
...
Use the channel’s past _N_ uploads for the z-score baseline. Optionally expose a rolling seven-day window per upload to help contextualize spikes or dips.
...
- **Early Hook Score (EHS):** Uses YouTube's `relativeRetentionPerformance` metric, which is already normalized to "average performance for videos of similar length." This metric is on a 0-100 scale and represents retention performance relative to similar videos.

```
EHS = relativeRetentionPerformance (0-100 scale)
```

- **Combined RetentionQuality:**

For early channels (< 5 videos):

```
RetentionQuality = 0.6 * AVP + 0.4 * EHS  (absolute, not z-scored)
```

For established channels (≥ 5 videos):

```
RetentionQuality = z(0.6 * AVP + 0.4 * EHS)
```

**Rationale:** For early channels, YouTube's `relativeRetentionPerformance` serves as the baseline—it's already normalized against similar-length videos, providing a platform-standard reference. Once a channel has ≥ 5 videos, we switch to z-scoring against the channel's own history to track improvement relative to their own performance.

High AVP signals overall content strength, while EHS (via `relativeRetentionPerformance`) highlights retention performance relative to similar videos.
...
```
RetentionQuality = 0.6 * AVP + 0.4 * EHS  (absolute, not z-scored)
SGI_raw = 0.40 * z(VV7) + 0.45 * RetentionQuality + 0.15 * z(SPV)
```
...
Use rolling percentiles or min/max bounds of recent uploads to keep the displayed score stable and interpretable. Early channels use YouTube's `relativeRetentionPerformance` as the baseline (already normalized), while established channels use z-scores against their own history.
